...
class CollaborativeFiltering_user2(
  ...
  def this(input: RDD[(String, String, Double)]) =
    this(input, 100, 100, 0, 20, 800, 100, 100, 500, Similarity.COSINE)

  def this(input: RDD[(String, String, Double)], similarity: Similarity) =
    this(input, 100, 100, 0, 20, 800, 100, 100, 500, similarity)
  ...
  private def simPair(): RDD[(String, (String, Double))] = {
    val itemsCount = userHistory.flatMap {
      case (user, items) => items.map(item => (item._1, (item._2 * item._2, 1)))} //(tuid, (freq的平方，1))
      .reduceByKey((x, y) => (x._1 + y._1, x._2 + y._2)).filter(_._2._2 >= minItemHistory) //(tuid,(freq的平方的和，个数))
      .map(line => (line._1, line._2._1)).collectAsMap() //分母
