...
for i in {-1..-1};do
        ...
        echo ${ip}": is work"
        input_user_follow="/user/wangshuang/apps/xfollow/real_follow/dt=$day"

        input_user_history="/user/wangshuang/apps/xdata-2/user_history/dt=$day"
        prefix="tfollowwatch_"
        mail=jiangcx@tangdou.com
        outputdir="hdfs://10.42.31.63:8020/user/jiangcx/vector/follow_watch"
        output=$outputdir/day=$day
        hadoop fs -rmr $outputdir/day=$day/follow_watch
...
#        hadoop fs -mkdir $output
        ...
        spark-submit \
            --master yarn-cluster \
            --queue root.default \
            --class com.wifi.recommend.FollowWatch \
            ...
            --conf spark.app.name="FollowWatch: online $day" \
            recall-rank-1.0-SNAPSHOT.jar \
            $input_user_follow $output $prefix $oldday $day $input_user_history
...
#hadoop fs -cat $output/follow_watch/* > simlist_user.$day
